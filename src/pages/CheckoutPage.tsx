...
import { ChevronLeft, CreditCard, Check, AlertCircle, Plus, Pencil, Tag, X as XIcon } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const result = await couponService.validateCoupon(couponCode.trim(), subtotal + shipping);
                    ...
                    {appliedCoupon ? (
                      <div className="flex items-center justify-between bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg px-4 py-3">
                        ...
                        <button onClick={handleRemoveCoupon} aria-label="Remove coupon">
                          <XIcon size={16} className="text-green-700 dark:text-green-300 hover:text-red-500 transition-colors" />
                        </button>
                      </div>
                    ) : (
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-4 py-2 text-sm bg-primary text-primary-foreground rounded-lg font-semibold disabled:opacity-50 hover:opacity-90 transition-opacity"
                        >
                          {couponLoading ? '...' : 'Apply'}
                        </button>
                      </div>
