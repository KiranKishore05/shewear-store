...
export default function AdminPage() {
  const [isAdminLoggedIn, setIsAdminLoggedIn] = useState(
    () => sessionStorage.getItem('sw_admin_auth') === 'true'
  );
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        ...
        >
          <div className="bg-gradient-to-r from-primary to-purple-600 p-8 text-white text-center">
            <Lock size={40} className="mx-auto mb-3 opacity-90" />
            <h2 className="text-2xl font-serif font-bold">Admin Access</h2>
            <p className="text-white/80 text-sm mt-1">Enter your admin credentials to continue</p>
          </div>
          <form onSubmit={handleAdminLogin} className="p-8 space-y-5">
            ...
            {adminLoginError && (
              <p className="text-sm text-destructive font-medium">{adminLoginError}</p>
            )}
            <motion.button
